...
const { ShoppingCart, Menu, X, ChevronLeft, ChevronRight, Minus, Plus, Upload } = lucide;

// PRODUCT DATA - Replace this with your actual product information
// Upload your images to Imgur, ImgBB, or Cloudinary and paste the URLs here
...
const SariEcommerce = () => {
  ...
  // ADMIN MODE - Set to false before launching your website
  const ADMIN_MODE = true; // Change this to false to hide CSV upload for public

  // Generate mock products or use uploaded products
  const generateMockProducts = () => {
    ...
    const mockProducts = Array.from({ length: 156 - baseProducts.length }, (_, i) => {
      ...
      return {
        ...
        description: `Exquisite ${sariTypes[index % sariTypes.length]} sari in stunning ${colors[index % colors.length]} with intricate ${patterns[index % patterns.length]} work. Perfect for weddings, festivals, and special occasions. This handcrafted masterpiece features traditional craftsmanship with modern elegance.`
      };
    });
  ...
  const handleCSVUpload = async (event) => {
    ...
    const parsedProducts = dataLines.map((line, index) => {
      ...
      const imageUrls = images ? images.split('|').map(url => url.trim()) : [
        "https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=400&h=600&fit=crop&q=80"
      ];
      ...
      React.createElement('div', { className: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" },
        ...
        ADMIN_MODE && showUploadInfo && React.createElement('div', { className: "bg-rose-50 p-4 mb-4 rounded-lg border-2 border-rose-200" },
          ...
          React.createElement('p', { className: "text-sm text-gray-700 mb-3" },
            "Create a CSV file with these columns: id, name, originalPrice, discountedPrice, images, tags, description"
          ),
        ...
        mobileMenuOpen && React.createElement('nav', { className: "md:hidden pb-4 space-y-2" },
          ...
          ADMIN_MODE && React.createElement('div', { className: "px-4 py-2" },
            React.createElement('button', {
              onClick: () => setShowUploadInfo(!showUploadInfo),
              className: "flex items-center gap-2 px-3 py-1.5 bg-rose-100 text-rose-900 rounded-lg hover:bg-rose-200 transition"
            },
              React.createElement(Upload, { size: 18 }),
              React.createElement('span', { className: "text-sm" }, "Upload CSV")
            )
  ...
  const Footer = () => (
    React.createElement('footer', { className: "bg-rose-900 text-white mt-16" },
      React.createElement('div', { className: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12" },
        ...
        React.createElement('div', { className: "text-center mb-8" },
          React.createElement('h4', { className: "font-semibold text-lg mb-6" }, "Quick Links"),
          React.createElement('div', { className: "grid grid-cols-3 gap-8 max-w-2xl mx-auto" },
            React.createElement('div', null,
              React.createElement('button', { onClick: () => setCurrentPage('home'), className: "block text-rose-200 hover:text-white mb-3 mx-auto" }, "Shop"),
              React.createElement('button', { onClick: () => setCurrentPage('about'), className: "block text-rose-200 hover:text-white mx-auto" }, "About Us")
            ),
            React.createElement('div', null,
              React.createElement('p', { className: "font-semibold text-white mb-3" }, "Support"),
              React.createElement('button', { onClick: () => setCurrentPage('faq'), className: "block text-rose-200 hover:text-white mb-3 mx-auto" }, "FAQ"),
              React.createElement('button', { onClick: () => setCurrentPage('contact'), className: "block text-rose-200 hover:text-white mx-auto" }, "Contact")
            ),
            React.createElement('div', null,
              React.createElement('p', { className: "font-semibold text-white mb-3" }, "Legal"),
              React.createElement('button', { onClick: () => setCurrentPage('policy'), className: "block text-rose-200 hover:text-white mx-auto" }, "Policy")
            )
          )
        ),
  ...
  const ProductCard = ({ product }) => {
    ...
    return React.createElement('div', { 
      ...
      React.createElement('div', { className: "aspect-[2/3] overflow-hidden bg-gray-100 relative" },
        ...
        discountPercent > 0 && React.createElement('div', { className: "absolute top-2 right-2 bg-rose-600 text-white px-2 py-1 rounded-md text-sm font-semibold" },
          `${discountPercent}% OFF`
        )
      ),
      ...
      React.createElement('div', { className: "grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16" },
        React.createElement('div', { className: "space-y-4" },
          React.createElement('div', { className: "aspect-[2/3] overflow-hidden rounded-lg bg-gray-100 relative" },
            ...
            discountPercent > 0 && React.createElement('div', { className: "absolute top-4 right-4 bg-rose-600 text-white px-4 py-2 rounded-lg text-lg font-bold shadow-lg" },
              `${discountPercent}% OFF`
            )
          ),
          React.createElement('div', { className: "grid grid-cols-3 gap-4" },
            selectedProduct.images.map((img, idx) =>
              React.createElement('div', {
                ...
                className: `aspect-[2/3] overflow-hidden rounded-lg cursor-pointer border-2 ${
                  selectedImage === idx ? 'border-rose-900' : 'border-transparent'
                }`
        ...
        React.createElement('div', { className: "space-y-6" },
          ...
          React.createElement('div', { className: "flex items-center gap-3" },
            ...
            selectedProduct.originalPrice !== selectedProduct.discountedPrice && React.createElement(React.Fragment, null,
              ...
              React.createElement('span', { className: "bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-semibold" },
          ...
          React.createElement('div', { className: "flex flex-wrap gap-2" },
            selectedProduct.tags.map(tag =>
              React.createElement('span', { key: tag, className: "px-3 py-1 bg-rose-100 text-rose-900 rounded-full text-sm" }, tag)
            )
          ),
  ...
  return React.createElement('div', { className: "min-h-screen bg-gray-50 flex flex-col" },
    ...
    React.createElement('main', { className: "flex-1" },
      ...
      currentPage === 'about' && React.createElement(InfoPage, { 
        ...
        content: React.createElement('p', { className: "text-gray-600 leading-relaxed" },
          "Suvarna Sarees is dedicated to preserving and celebrating the timeless elegance of traditional Indian sarees. Our collection features handpicked sarees from master weavers across India, each piece telling a unique story of craftsmanship and heritage. We believe in bringing you authentic, high-quality sarees that honor tradition while embracing contemporary aesthetics."
        )
      }),
...
root.render(React.createElement(SariEcommerce));', { className: "flex items-center gap-2" },
